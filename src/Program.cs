...
namespace JCruzPorcel.Inscipcion_Colegio.src // Utilizo este namespace para tener mas organización y que se vea quien es el autor.
{
    ...
    internal class Program // La clase Program se declara como "internal" para restringir su acceso desde programas o clases externas al proyecto actual. Esta clase contiene la lógica principal del programa.
    {
        ...
        static void Main(string[] args)
        {
            ...
            string anguilFilePath = @Path.Combine(basePath, FOLDER, ANGUIL_FILE_PATH);        // No es necesario, pero lo hice para evitar posibles errores con las extenciones.                                                                                               
            string toayFilePath = @Path.Combine(basePath, FOLDER, TOAY_FILE_PATH);           // Por ejemplo: .xlsx
            ...
            AsignarAlumnosAColegios(alumnos, colegios);

            foreach (Colegio colegio in colegios)
            {
                string outputFileName = $"Vacantes_{colegio.Nombre}.xlsx";
                CrearArchivoVacantesColegio(colegio.AlumnosAsignados, outputFileName, outputFilePath);

                Console.WriteLine($"Proceso finalizado. Se han generado los archivos de vacantes para el colegio {colegio.Nombre}.");
            ...
            Console.WriteLine("Se generaron todos los archivos de vacantes con EXITO.");
            ...
            static void AsignarAlumnosAColegios(List<Alumno> alumnos, List<Colegio> colegios)
            {
                foreach (Alumno alumno in alumnos)
                {
                    foreach (Colegio colegio in colegios)
                    {
                        if (Regex.IsMatch(alumno.Preferencia, @"\b" + Regex.Escape(colegio.Nombre) + @"\b", RegexOptions.IgnoreCase) // Esto verifica que los nombres de los colegios sean los mismos que su preferencia
                                                                                                                                     // comprobando si contiene en alguna parte la misma cadena de strings
                            && colegio.TieneVacantes(alumno.Grado))
                        {
